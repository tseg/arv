        ...
        <div class="box-body">
          <div class="row">
            <div class="col-md-12">
            	...
            	<?php
					$proectid = $_GET['task_id'];
					$stmt = $DB_con->prepare('SELECT * FROM tbl_viewer_sketch where Project_Id = '.$proectid.'');
					$stmt->execute();
					...
					if ($stmt->rowCount() > 0) {
						...
						if($cstmt->rowCount() > 0){
							...
							if($crow['Rate_Up'] == NULL and $crow['Rate_Down'] == NULL){
								$rating = 0;
							}else{
								$rating = 1;
							}
        		...
        		?>
        			<tr>
        				<td><?php echo $i; ?></td>
        				<td><?php echo $userobj->firstname." ".$userobj->lastname; ?></td>
        				<td><?php echo $row['Date_Time']; ?></td>
        				<?php 
        				if($rating == 0){
						
							echo "<td><a href='judge_task_images_rating.php?sketch_id=".$row['id']."'>Process Rating</a></td>";
						
        				}else{
        					
        					echo "<td><a href='judge_task_images_rating.php?sketch_id=".$row['id']."'>Rating is Done</a></td>";
    			...
    			?>
        				<tr>
        					<td><span class="glyphicon glyphicon-info-sign"></span> &nbsp; No Data Found ...</td>
        				</tr>
        	<?php
   				 }
    		?>
    		...
    		?>
    			<span class="glyphicon glyphicon-info-sign"></span> &nbsp; Wrong URL Please try again ...</td>
    		<?php
    		}
    		?>
</div>
...
<?php
//include'all_image_uploaded_by_viewers.php';
